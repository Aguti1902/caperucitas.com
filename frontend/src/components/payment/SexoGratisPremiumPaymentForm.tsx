...
export default function SexoGratisPremiumPaymentForm({ onSuccess, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        const response = await api.post('/payments/sexo-gratis-premium/payment-intent')
        if (cancelled) return
        if (response.data?.clientSecret) {
          setClientSecret(response.data.clientSecret)
        } else if (response.data?.isPremium || response.data?.profile) {
          // Entorno sin Stripe: activado directo
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <p className="text-gray-500 text-xs text-center">
        Premium 3 meses: teléfono/WhatsApp públicos, mejor posición y carrusel (~10 km).
      </p>
    </form>
  )
}
