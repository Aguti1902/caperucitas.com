...
import { SPANISH_MUNICIPALITIES, getCityPath, SITE_URL } from '@/utils/citySeo'
...
export default function CitiesDirectoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return SPANISH_MUNICIPALITIES.slice(0, 80)
    return SPANISH_MUNICIPALITIES.filter(
      (m) => m.name.toLowerCase().includes(q) || m.slug.includes(q.replace(/\s+/g, '-'))
    ).slice(0, 100)
  }, [search])
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <SeoHead
        ...
        description="Directorio de putas, escorts y sexo gratis por ciudad en España. Encuentra perfiles cerca de ti en Madrid, Barcelona, Valencia y más de 100 ciudades."
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        <header className="space-y-2">
          ...
          <p className="text-gray-600 text-xs">
            {search ? `${filtered.length} resultados` : `Mostrando 80 de ${SPANISH_MUNICIPALITIES.length} municipios — escribe para buscar`}
          </p>
        </header>
